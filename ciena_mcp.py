...
Ejemplos:
    ...
    python ciena_mcp.py --network-constructs --ip-startswith 10.20
    python ciena_mcp.py --network-constructs --name-contains R01 --name-endswith REP
...
def print_network_constructs(constructs, raw):
    ...
    for nc in constructs:
        attrs = nc["attributes"]
        display_name = attrs.get("displayData", {}).get("displayName", "")
        print(f"{attrs.get('name', ''):<25} {display_name:<25} {attrs.get('ipAddress', ''):<25} {nc['id']:<20}")
...
def build_parser():
    ...
    parser.add_argument(
        ...
        help="Filtra resultados cuyo nombre contenga TEXT (case-insensitive). "
             "Aplica a --network-constructs, --equipment y --facility-resources.",
    )
    parser.add_argument(
        "--name-startswith",
        metavar="TEXT",
    ...
    parser.add_argument(
        "--name-endswith",
        metavar="TEXT",
        help="Filtra resultados cuyo nombre termine con TEXT (case-insensitive), p.ej. REP.",
    )
    parser.add_argument(
        "--ip-startswith",
        metavar="PREFIX",
        help="Filtra network constructs cuya IP empiece con PREFIX, p.ej. 10.20 (sólo --network-constructs).",
    )
    parser.add_argument(
        "--link-label-contains",
        metavar="TEXT",
        help="Filtra fres cuyo linkLabel contenga TEXT, p.ej. R01-LINK-REP (sólo --facility-resources).",
    )
    parser.add_argument(
        "--user-label-contains",
        metavar="TEXT",
        help="Filtra fres cuyo userLabel contenga TEXT, p.ej. SITE-A (sólo --facility-resources).",
    )
    parser.add_argument(
        "--layer-rate",
        metavar="VALUE",
...
def main():
    ...
    actions = [args.network_constructs, bool(args.equipment), bool(args.facility_resources), args.metrics, args.logout]
    if not any(actions):
        parser.print_help()
        return
    ...
    if args.network_constructs:
        constructs = apply_filters(nsi.network_constructs(), args, nc_names, nc_ip)
